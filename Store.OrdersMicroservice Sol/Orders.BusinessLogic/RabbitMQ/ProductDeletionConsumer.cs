...
{
    public class ProductDeletionConsumer : IProductDeletionConsumer, IDisposable
    {
        ...
        public ProductDeletionConsumer(IConfiguration configuration,
        ...
        public void Consume()
        {
            ...
            consumer.ReceivedAsync += async (model, eventArgs) =>
            {
                ...
                if (productDeletionMessage is null)
                {
                    _logger.LogWarning("[RabbitMQ] Deserialized message is null.");
                    return;
                }

                _logger.LogInformation($"[RabbitMQ] Product Deleted: {productDeletionMessage.ProductId} - {productDeletionMessage.Name}");
